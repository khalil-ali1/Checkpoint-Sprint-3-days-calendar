  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (dayEventsMap[day]) {
      dayEventsMap[day].forEach((event) => {
        ...
        // Create an interactive button instead of a hyperlink
        const eventBtn = document.createElement("button");
        eventBtn.className = "event-btn";
        eventBtn.type = "button";
        eventBtn.textContent = event.name;

        // Set ARIA attributes to tell screen readers a dialog modal will open
        eventBtn.setAttribute("aria-haspopup", "dialog");
...
function showEventDetailsModal(event, description) {
  ...
  // Build the dialog layout container box with ARIA properties for Lighthouse 100% Accessibility
  ...
  // Modal destruction events (Closing behavior)
  ...
  // Append elements together and inject into the document body
  ...
  // Instantly focus the close button so screen readers read the popup immediately
  closeBtn.focus();
}
